...
from rcp_rclm_runtime.replay.reference import run_reference_phase8_roundtrip
...
def main() -> int:
    ...
    evidence = run_reference_phase8_roundtrip(
        outdir,
        fixture_lean_evidence,
        fixture_lean_evidence,
    )
    payload = evidence.to_json()
    payload["roundtrip_hash"] = evidence.roundtrip_hash
    payload["lean_mode"] = "deterministic_fixture_for_platform_testing"
    summary_path = outdir / "phase_8_reference_suite.json"
    summary_path.write_bytes(canonical_json_bytes(payload))
