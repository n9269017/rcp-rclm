...
def run_test_suite(package_root: Path, output_path: Path) -> int:
    ...
    tests_root = resolved_root / "tests_phase5"
    if not tests_root.is_dir():
        raise FileNotFoundError(f"Phase 5 tests directory is missing: {tests_root}")
    command = [
        sys.executable,
        "-m",
        "unittest",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the deterministic Phase 5A reference-generator unit suite."
    )
