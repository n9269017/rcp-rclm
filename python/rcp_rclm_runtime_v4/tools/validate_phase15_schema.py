...
def main() -> int:
    ...
    errors = sorted(Draft202012Validator(schema).iter_errors(instance), key=lambda item: tuple(item.absolute_path))
    hash_field = "manifest_hash" if isinstance(instance, dict) and str(instance.get("schema_id", "")).endswith("bundle_manifest.v1") else "report_hash"
    hash_valid = False
    ...
    report = {
        "schema_id": "runtime.v4.phase15.schema_validation.v1",
        "ok": not errors and hash_valid,
        "schema": args.schema.name,
        "instance": args.instance.name,
        "schema_valid": not errors,
        "content_hash_valid": hash_valid,
        "errors": [{"path": "/" + "/".join(str(part) for part in error.absolute_path), "message": error.message} for error in errors],
    }
