...
class Phase15BundleFile:
    path: str
    size_bytes: int
    sha256: str

    schema_id: ClassVar[str] = "runtime.v4.phase15.bundle_file.v1"

    def __post_init__(self) -> None:
        validate_semantic_path(self.path)
        if isinstance(self.size_bytes, bool) or self.size_bytes < 0:
            raise SchemaValidationError(
                "phase15.bundle_file.size_bytes",
    ...
    @classmethod
    def from_json(cls, value: object) -> Phase15BundleFile:
        if not isinstance(value, dict):
            raise SchemaValidationError("phase15.bundle_file", "expected object")
        return cls(
            path=str(value["path"]),
...
@dataclass(frozen=True, slots=True)
class Phase15BundleManifest:
    source_head: str
    source_tree: str
    trajectory_report_hash: str
    files: Sequence[Phase15BundleFile]
    empty_directories: Sequence[str]

    schema_id: ClassVar[str] = "runtime.v4.phase15.bundle_manifest.v1"
    ...
    def __post_init__(self) -> None:
        ...
        if empty != tuple(sorted(empty, key=lambda item: item.encode("utf-8"))):
            raise SchemaValidationError(
                "phase15.bundle.empty_directories",
    ...
    @classmethod
    def from_json(cls, value: object) -> Phase15BundleManifest:
        if not isinstance(value, dict):
            raise SchemaValidationError("phase15.bundle", "expected object")
...
def _empty_directories(root: Path) -> tuple[str, ...]:
    ...
    for path in sorted(root.rglob("*"), key=lambda item: item.as_posix().encode("utf-8")):
        if path.is_symlink():
            raise SchemaValidationError("phase15.bundle", "symlinks are forbidden")
        if path.is_dir() and not any(path.iterdir()):
            result.append(path.relative_to(root).as_posix())
    return tuple(result)


def build_phase15_bundle(
    *,
    campaign_root: Path,
    bundle_root: Path,
    ...
    if output.exists():
        raise FileExistsError(f"Phase 15 bundle already exists: {output}")
    trajectory_value = load_json_strict(
        (source / "phase15_trajectory.json").read_bytes(),
        require_canonical=True,
    )
    trajectory = Phase15TrajectoryReport.from_json(trajectory_value)
    ...
    manifest = Phase15BundleManifest(
...
def verify_phase15_bundle(bundle_root: Path) -> Phase15BundleManifest:
    root = bundle_root.resolve(strict=True)
    manifest_value = load_json_strict(
        (root / "manifest.json").read_bytes(),
        require_canonical=True,
    )
    manifest = Phase15BundleManifest.from_json(manifest_value)
