...
def _pinned_gate_b_compiler(
    *,
    repo_root: Path,
    lean_project_root: Path,
) -> LeanCompiler:
    repository_root = repo_root.resolve(strict=True)
    execution_root = lean_project_root.resolve(strict=True)
    expected_root = (
        repository_root / "lean/rcp_rclm_formal_core_v4"
    ).resolve(strict=True)
    if execution_root != expected_root:
        raise ValueError(
            "outer verification requires the pinned Formal Core v4 "
            "execution root"
        )
    pinned_project = PinnedLeanProject.discover(repository_root)
    execution_project = replace(pinned_project, root=execution_root)
    return LeanCompiler(
...
def verify_outer_envelope(
    ...
    *,
    ...
    if lean_project_root is None:
        ...
        compiler = _pinned_gate_b_compiler(
            repo_root=repo_root,
