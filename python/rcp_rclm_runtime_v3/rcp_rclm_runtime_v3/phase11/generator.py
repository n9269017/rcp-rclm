...
def _policy_object(package_root: Path, relative_path: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise SchemaValidationError("phase11.generator.policy", "expected policy object")
    return value


def _active_policy_bindings(package_root: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if generator.get("generator_profile") != PHASE11_GENERATOR_PROFILE:
        raise SchemaValidationError("phase11.generator.policy", "generator profile mismatch")
    if generator.get("proposal_protocol_hash") != PROPOSAL_PROTOCOL_HASH:
        raise SchemaValidationError("phase11.generator.policy", "proposal protocol mismatch")
    if generator.get("learned_proposal_authority") is not True:
        raise SchemaValidationError("phase11.generator.policy", "proposal authority is disabled")
    if planner.get("proposal_protocol_hash") != PROPOSAL_PROTOCOL_HASH:
        raise SchemaValidationError("phase11.planner.policy", "proposal protocol mismatch")
    if planner.get("typed_mutation_program_required") is not True:
        raise SchemaValidationError("phase11.planner.policy", "typed output is not required")
...
def generate_typed_mutation_program(
    ...
    if generator_input.active_package_hash != manifest.package_hash:
        raise SchemaValidationError("phase11.generator.input", "active package hash mismatch")
    if generator_input.model_identity_hash != manifest.model_identity_hash:
        raise SchemaValidationError("phase11.generator.input", "model identity hash mismatch")
    if generator_input.active_generator_hash != manifest.generator_policy_hash:
        raise SchemaValidationError("phase11.generator.input", "active generator hash mismatch")
    if generator_input.active_planner_hash != manifest.planner_policy_hash:
        raise SchemaValidationError("phase11.generator.input", "active planner hash mismatch")
    if generator_input.proposal_protocol_hash != PROPOSAL_PROTOCOL_HASH:
        raise SchemaValidationError("phase11.generator.input", "proposal protocol hash mismatch")
    if generator_input.objective_hash != phase11_objective_hash():
        raise SchemaValidationError("phase11.generator.input", "objective hash mismatch")

    bank_start, bank_capacity = _bank_for_invocation(generator_input.invocation_index)
    output = bytearray()
    ...
    if not stopped:
        raise SchemaValidationError("phase11.generator.output", "model output did not terminate")
    program = parse_typed_mutation_program(bytes(output))

    expected_length_field = (
        "rejected_program_length" if generator_input.invocation_index == 0 else "accepted_program_length"
    )
    expected_hash_field = (
        "rejected_program_sha256" if generator_input.invocation_index == 0 else "accepted_program_sha256"
    )
    if generator_policy.get(expected_length_field) != len(output):
        raise SchemaValidationError("phase11.generator.policy", "output length binding mismatch")
    raw_hash = sha256_hex(bytes(output))
    if generator_policy.get(expected_hash_field) != raw_hash:
        raise SchemaValidationError("phase11.generator.policy", "output hash binding mismatch")
    if generator_policy.get("generation") != ACTIVE_GENERATOR_GENERATION:
        raise SchemaValidationError("phase11.generator.policy", "active generation mismatch")
    if planner_policy.get("generation") != ACTIVE_PLANNER_GENERATION:
        raise SchemaValidationError("phase11.planner.policy", "active generation mismatch")
...
def _expected_components(update_classes: Sequence[str]) -> tuple[str, ...] | None:
    components: list[str] = []
...
def validate_generated_program(
    ...
    bindings: dict[str, bool] = {
        ...
        "proposal_protocol_bound": generator_input.proposal_protocol_hash == PROPOSAL_PROTOCOL_HASH,
        "objective_bound": generator_input.objective_hash == phase11_objective_hash(),
    ...
    if selected & set(FORBIDDEN_UPDATE_CLASSES) or not selected <= set(ALLOWED_UPDATE_CLASSES):
    ...
    if expected_components is None or expected_components != tuple(
