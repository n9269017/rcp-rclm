...
PHASE12D_CONTROLLER_ENVIRONMENT_HASH: Final[str] = canonical_json_hash(
    {
...
class Phase12DAuthoritativeVerification:
    ...
    schema_id: ClassVar[str] = "runtime.v3.phase12d.authoritative_verification.v1"
    ...
    def frontier_transition_accepted(self) -> bool:
        return (
            ...
            and not self.active_phase12d.solved
...
def verify_phase12d_candidate(
    ...
    *,
    ...
    information = build_phase12d_information_report(active_root, candidate_root)
    if information.report_hash != reference.semantic_candidate.information_report.report_hash:
        raise ValueError("authoritative Phase 12D information report differs")

    active_state = reference.phase12c.semantic_candidate.candidate_state
    ...
    checker_request = Phase3CheckerRequest(
        ...
        resource_record=reference_resource_record(
            budget_units=1,
            consumed_units=1,
            environment_hash=PHASE12D_CONTROLLER_ENVIRONMENT_HASH,
        ),
    ...
    evidence = Phase12DAuthoritativeVerification(
        ...
        generator_projection_matches=reference.phase6.generator_projection_matches,
        planner_projection_matches=reference.phase6.planner_projection_matches,
        forbidden_training_modules_loaded=_forbidden_training_modules(),
    )
    if not evidence.accepted:
        raise ValueError("Phase 12D authoritative verification did not accept")
    if not reference.lifecycle_transition.accepted:
        raise ValueError("Phase 12D Gate D transition is not accepted")
    return evidence
...
def _promotion_stages(
    reference: Phase12DReference,
    verification: Phase12DAuthoritativeVerification,
) -> Sequence[Phase7StageResult]:
    realization = reference.phase6.phase6.report.realization
    if realization is None:
        raise ValueError("Phase 12D Phase 6 realization is unavailable")
    return (
        _stage(
            "generator",
            "pass",
            {
                "proposal_source": "active_m2_generation2_generator_planner",
        ...
        _stage(
            ...
            "pass",
            {
                ...
                "affected_components": ["generator_policy", "planner_policy"],
            },
        ),
        _stage(
            "realization",
...
@dataclass(frozen=True, slots=True)
class Phase12DPromotionEvidence:
    phase12c_prefix: Phase12CPromotionEvidence
    reference: Phase12DReference
    verification: Phase12DAuthoritativeVerification
    promotion_attempt: Phase7AttemptReport
    promotion: Phase7PromotionCommit
    m2_store_package_hash: str
    ...
    schema_id: ClassVar[str] = "runtime.v3.phase12d.promotion.v1"

    @property
    def accepted(self) -> bool:
        pointer = self.promotion.snapshot.pointer
        active_manifest = self.reference.phase12c.semantic_candidate.manifest
        candidate_manifest = self.reference.semantic_candidate.manifest
        return (
            self.phase12c_prefix.accepted
            ...
            and self.progress_ledger.generator_invocations == 5
            and self.progress_ledger.rejected_attempts == 2
            and self.progress_ledger.candidate_realizations == 3
            and self.progress_ledger.candidate_evaluations == 3
            and self.progress_ledger.accepted_promotions == 3
            and self.progress_ledger.frontier_expansions == 3
    ...
    def to_json(self) -> dict[str, object]:
        return {
            "schema_id": self.schema_id,
            "accepted": self.accepted,
            "phase12c_prefix_promotion_hash": self.phase12c_prefix.report_hash,
            ...
            "m2_store_package_hash": self.m2_store_package_hash,
            ...
            "active_generator_hash": self.reference.phase12c.semantic_candidate.manifest.generator_policy_hash,
            "successor_generator_hash": self.reference.semantic_candidate.manifest.generator_policy_hash,
            "active_planner_hash": self.reference.phase12c.semantic_candidate.manifest.planner_policy_hash,
            "successor_planner_hash": self.reference.semantic_candidate.manifest.planner_policy_hash,
            "successor_proposal_protocol_hash": (
                self.reference.semantic_candidate.candidate_state.self_hosting.proposal_protocol_hash
            ),
            "progress_ledger": self.progress_ledger.to_json(),
            "manual_repair_count": 0,
            "recursive_use_of_m2_generator": True,
            "generation3_generator_planner_installed": True,
        }


def promote_phase12d_candidate(
    reference: Phase12DReference,
    verification: Phase12DAuthoritativeVerification,
    *,
    store_root: Path,
    ...
    if not reference.accepted or not verification.accepted:
        raise ValueError("Phase 12D lifecycle is not eligible for promotion")

    phase12c_verification = verify_phase12c_candidate(
        reference.phase12c,
        repo_root=repo_root,
        lean_project_root=lean_project_root,
    )
    phase12c_prefix = promote_phase12c_candidate(
        reference.phase12c,
        phase12c_verification,
        store_root=store_root,
        evidence_root=evidence_root / "phase12c-prefix",
        repo_root=repo_root,
        lean_project_root=lean_project_root,
    )
    policy = phase12b_phase7_policy()
    snapshot = load_active_phase7_store(store_root, policy)
    if snapshot.pointer != phase12c_prefix.promotion.snapshot.pointer:
        raise ValueError("reopened M2 store pointer differs from Phase 12C prefix")
    if snapshot.pointer.ledger_sequence_number != 4:
        raise ValueError("Phase 12D prefix ledger sequence is not four")
    m2_hash = snapshot.pointer.active_package_hash
    budget = phase12b_phase7_budget()
    run_id = phase7_run_id(
        run_label="phase12-self-hosted-recursion-generation3-v1",
        active_pointer_hash=snapshot.pointer.pointer_hash,
        policy_hash=policy.policy_hash,
        budget_hash=budget.budget_hash,
    )
    root = (evidence_root / "phase12d").resolve(strict=False)
    if root.exists():
        raise FileExistsError(f"Phase 12D evidence root already exists: {root}")
    ...
    promotion_attempt = Phase7AttemptReport(
        ...
        artifact_hashes=FrozenHashMap.from_mapping(
            promotion_hashes,
            "phase12d.promotion_artifact_hashes",
        ),
    )
    ...
    if reopened.pointer != promotion.snapshot.pointer:
        raise ValueError("reopened M3 active pointer differs")
    ...
    if installed_manifest.package_hash != reference.semantic_candidate.manifest.package_hash:
        raise ValueError("promoted M3 package contains the wrong semantic package")
    ...
    expected_generator_bytes_hash = sha256_hex(
        (reference.semantic_candidate.root / "policies/generator_policy.json").read_bytes()
    )
    expected_planner_bytes_hash = sha256_hex(
        (reference.semantic_candidate.root / "policies/planner_policy.json").read_bytes()
    )
    if installed_generator_bytes_hash != expected_generator_bytes_hash:
        raise ValueError("installed M3 generator bytes differ")
    if installed_planner_bytes_hash != expected_planner_bytes_hash:
        raise ValueError("installed M3 planner bytes differ")
    ...
    result = Phase12DPromotionEvidence(
        phase12c_prefix=phase12c_prefix,
        reference=reference,
        verification=verification,
        promotion_attempt=promotion_attempt,
        promotion=promotion,
        m2_store_package_hash=m2_hash,
    ...
    if not result.accepted:
        raise ValueError("Phase 12D promotion did not close")
    _write_json(root / "phase12d_promotion_report.json", result.to_json())
    return result


@dataclass(frozen=True, slots=True)
class Phase12DClosureEvidence:
    reference: Phase12DReference
    verification: Phase12DAuthoritativeVerification
    promotion: Phase12DPromotionEvidence

    schema_id: ClassVar[str] = "runtime.v3.phase12d.closure.v1"
    ...
    def to_json(self) -> dict[str, object]:
        return {
            ...
            "m2_to_m3_frontier_expansion": True,
            "m2_to_m3_generator_planner_self_modification": True,
            "m2_to_m3_atomic_promotion": True,
            "generation3_generator_planner_installed": True,
            "frontier_cardinality_before": 5,
            "frontier_cardinality_after": 6,
            "accepted_phase12_promotions": 3,
            "rejected_phase12_attempts": 2,
            "manual_repairs": 0,
            "heldout_material_consumed": False,
            "phase12d_generator_planner_promotion_closed": self.accepted,
            "phase12_exit_closed": False,
            "next_transition": "M3_to_M4_architecture_adapter_optimizer",
        }
