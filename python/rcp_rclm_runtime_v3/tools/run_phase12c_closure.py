...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rcp-rclm-phase12c-closure-") as temporary:
        root = Path(temporary)
        reference = build_phase12c_reference(
            root / "reference",
            repo_root=args.repo_root,
        )
        verification = verify_phase12c_candidate(
            reference,
            repo_root=args.repo_root,
            lean_project_root=args.lean_project_root,
        )
        promotion = promote_phase12c_candidate(
            reference,
            verification,
        ...
        closure = Phase12CClosureEvidence(
            reference=reference,
            verification=verification,
            promotion=promotion,
        )
        if not closure.accepted:
            raise ValueError("Phase 12C authoritative closure did not accept")
