...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rcp-rclm-phase12b-closure-") as temporary:
        root = Path(temporary)
        reference = build_phase12b_reference(
            root / "reference",
            repo_root=args.repo_root,
        )
        verification = verify_phase12b_candidate(
            reference,
            repo_root=args.repo_root,
            lean_project_root=args.lean_project_root,
        )
        promotion = promote_phase12b_candidate(
            reference,
            verification,
            store_root=root / "store",
            evidence_root=root / "promotion_evidence",
        )
        closure = Phase12BClosureEvidence(
            reference=reference,
            verification=verification,
            promotion=promotion,
        )
        if not closure.accepted:
            raise ValueError("Phase 12B authoritative closure did not accept")
